          ...
          {books.map(b => (
            <div key={b._id} className="bg-white rounded-3xl border border-gray-100 shadow-sm overflow-hidden hover:shadow-md transition-shadow group">
              <div className="relative aspect-square bg-gray-100 overflow-hidden">
                ...
                <span className="absolute top-3 left-3 px-2.5 py-1 bg-brand-blue text-white text-xs font-bold rounded-full">{typeLabel(b.type)}</span>
              </div>
              <div className="p-4">
                ...
                <div className="mt-3 flex gap-2">
                  <button onClick={() => window.location.hash = `#offer-detail?id=${b._id}`} className="flex-[2] py-2 bg-brand-blue/5 hover:bg-brand-blue text-brand-blue hover:text-white rounded-xl text-xs font-bold transition-all">
                    Consultez
                  </button>
